...
func NewClient(mchId, apiKey string) (client *Client) {
	if mchId != paymod.NULL && apiKey != paymod.NULL {
		return &Client{
			MchId:  mchId,
			ApiKey: apiKey,
		}
...
func (q *Client) MicroPay(bm paymod.BodyMap) (qqRsp *MicroPayResponse, err error) {
...
func (q *Client) Reverse(bm paymod.BodyMap) (qqRsp *ReverseResponse, err error) {
...
func (q *Client) UnifiedOrder(bm paymod.BodyMap) (qqRsp *UnifiedOrderResponse, err error) {
...
func (q *Client) OrderQuery(bm paymod.BodyMap) (qqRsp *OrderQueryResponse, err error) {
...
func (q *Client) CloseOrder(bm paymod.BodyMap) (qqRsp *CloseOrderResponse, err error) {
...
func (q *Client) Refund(bm paymod.BodyMap, certFilePath, keyFilePath, pkcs12FilePath interface{}) (qqRsp *RefundResponse, err error) {
...
func (q *Client) RefundQuery(bm paymod.BodyMap) (qqRsp *RefundQueryResponse, err error) {
	err = bm.CheckEmptyError("nonce_str")
	if err != nil {
		return nil, err
	}
...
func (q *Client) StatementDown(bm paymod.BodyMap) (qqRsp string, err error) {
	...
	if billType != "ALL" && billType != "SUCCESS" && billType != "REFUND" && billType != "RECHAR" {
		return paymod.NULL, errors.New("bill_type error, please reference: https://qpay.qq.com/buss/wiki/38/1209")
	}
...
func (q *Client) AccRoll(bm paymod.BodyMap) (qqRsp string, err error) {
	...
	if accType != "CASH" && accType != "MARKETING" {
		return paymod.NULL, errors.New("acc_type error, please reference: https://qpay.qq.com/buss/wiki/38/3089")
	}
...
// 向QQ发送请求
func (q *Client) doQQ(bm paymod.BodyMap, url string, tlsConfig *tls.Config) (bs []byte, err error) {
	bm.Set("mch_id", q.MchId)
	if bm.Get("fee_type") == paymod.NULL {
		bm.Set("fee_type", "CNY")
	}
