...
func checkCertFilePath(certFilePath, keyFilePath, pkcs12FilePath interface{}) error {
	if certFilePath != nil && keyFilePath != nil && pkcs12FilePath != nil {
		if v, ok := certFilePath.(string); !ok || v == paymod.NULL {
			return errors.New("certFilePath not string type or is null string")
		}
		if v, ok := keyFilePath.(string); !ok || v == paymod.NULL {
			return errors.New("keyFilePath not string type or is null string")
		}
		if v, ok := pkcs12FilePath.(string); !ok || v == paymod.NULL {
...
// 获取微信支付正式环境Sign值
func getReleaseSign(apiKey string, signType string, bm paymod.BodyMap) (sign string) {
	var h hash.Hash
...
// 获取微信支付沙箱环境Sign值
func getSignBoxSign(mchId, apiKey string, bm paymod.BodyMap) (sign string, err error) {
	var (
...
func getSanBoxSignKey(mchId, nonceStr, sign string) (key string, err error) {
	reqs := make(paymod.BodyMap)
	reqs.Set("mch_id", mchId)
	reqs.Set("nonce_str", nonceStr)
	reqs.Set("sign", sign)

	keyResponse := new(getSignKeyResponse)
	_, errs := paymod.NewHttpClient().Type(paymod.TypeXML).Post(sandboxGetSignKey).SendString(generateXml(reqs)).EndStruct(keyResponse)
	if len(errs) > 0 {
		return paymod.NULL, errs[0]
	}
	if keyResponse.ReturnCode == "FAIL" {
		return paymod.NULL, errors.New(keyResponse.ReturnMsg)
	}
	return keyResponse.SandboxSignkey, nil
}

// 生成请求XML的Body体
func generateXml(bm paymod.BodyMap) (reqXml string) {
