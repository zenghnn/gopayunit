...
func (w *Client) PostRequest(bm paymod.BodyMap, path string, tlsConfig *tls.Config) (bs []byte, err error) {
...
func GetParamSign(appId, mchId, apiKey string, bm paymod.BodyMap) (sign string) {
	bm.Set("appid", appId)
	bm.Set("mch_id", mchId)
	var (
		signType string
...
//    ApiKey：API秘钥值
//    返回参数 sign：通过Appid、MchId、ApiKey和BodyMap中的参数计算出的Sign值
func GetSanBoxParamSign(appId, mchId, apiKey string, bm paymod.BodyMap) (sign string, err error) {
	bm.Set("appid", appId)
	bm.Set("mch_id", mchId)
	bm.Set("sign_type", SignType_MD5)
	...
	if sandBoxApiKey, err = getSanBoxKey(mchId, paymod.GetRandomString(32), apiKey, SignType_MD5); err != nil {
		return
	}
	hashMd5 = md5.New()
...
//    返回参数bm：Notify请求的参数
//    返回参数err：错误信息
func ParseNotifyToBodyMap(req *http.Request) (bm paymod.BodyMap, err error) {
...
// 解密微信退款异步通知的加密数据
//    reqInfo：paymod.ParseRefundNotify() 方法获取的加密数据 req_info
//    apiKey：API秘钥值
...
func DecryptRefundNotifyReqInfo(reqInfo, apiKey string) (refundNotify *RefundNotify, err error) {
	if reqInfo == paymod.NULL || apiKey == paymod.NULL {
	...
	bs = paymod.PKCS7UnPadding(encryptionB)
...
//    iv：加密算法的初始向量，小程序获取到
//    sessionKey：会话密钥，通过  paymod.Code2Session() 方法获取到
...
func DecryptOpenDataToStruct(encryptedData, iv, sessionKey string, beanPtr interface{}) (err error) {
	if encryptedData == paymod.NULL || iv == paymod.NULL || sessionKey == paymod.NULL {
	...
	if len(plainText) > 0 {
		plainText = paymod.PKCS7UnPadding(plainText)
	}
...
//    sessionKey：会话密钥，通过  paymod.Code2Session() 方法获取到
//    文档：https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/signature.html
func DecryptOpenDataToBodyMap(encryptedData, iv, sessionKey string) (bm paymod.BodyMap, err error) {
	if encryptedData == paymod.NULL || iv == paymod.NULL || sessionKey == paymod.NULL {
...
func GetAppLoginAccessToken(appId, appSecret, code string) (accessToken *AppLoginAccessToken, err error) {
	...
	_, errs := paymod.NewHttpClient().Get(url).EndStruct(accessToken)
	if len(errs) > 0 {
		return nil, errs[0]
	}
...
func Code2Session(appId, appSecret, wxCode string) (sessionRsp *Code2SessionRsp, err error) {
	...
	_, errs := paymod.NewHttpClient().Get(url).EndStruct(sessionRsp)
	if len(errs) > 0 {
		return nil, errs[0]
	}
...
func GetOpenIdByAuthCode(appId, mchId, apiKey, authCode, nonceStr string) (openIdRsp *OpenIdByAuthCodeRsp, err error) {
	...
	_, errs := paymod.NewHttpClient().Type(paymod.TypeXML).Post(url).SendString(generateXml(bm)).EndStruct(openIdRsp)
	if len(errs) > 0 {
		return nil, errs[0]
	}
...
func GetAppletPaidUnionId(accessToken, openId, transactionId string) (unionId *PaidUnionId, err error) {
	...
	_, errs := paymod.NewHttpClient().Get(url).EndStruct(unionId)
	if len(errs) > 0 {
		return nil, errs[0]
	}
	return unionId, nil
...
func GetUserInfo(accessToken, openId string, lang ...string) (userInfo *UserInfo, err error) {
	...
	_, errs := paymod.NewHttpClient().Get(url).EndStruct(userInfo)
	if len(errs) > 0 {
		return nil, errs[0]
	}
	return userInfo, nil
