...
func GetRsaSign(bm paymod.BodyMap, signType string, t PKCSType, privateKey string) (sign string, err error) {
	var (
		block          *pem.Block
		h              hash.Hash
		key            *rsa.PrivateKey
	...
	if block, _ = pem.Decode([]byte(pk)); block == nil {
		return paymod.NULL, errors.New("pem.Decode：privateKey decode error")
	}
	...
	case PKCS8:
		...
		if !ok {
			return paymod.NULL, errors.New("parse PKCS8 key error")
		}
		key = pk8
	default:
		if key, err = x509.ParsePKCS1PrivateKey(block.Bytes); err != nil {
...
// 格式化请求URL参数
func FormatURLParam(body paymod.BodyMap) (urlParam string) {
	v := url.Values{}
