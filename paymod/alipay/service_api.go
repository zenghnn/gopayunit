...
func ParseNotifyToBodyMap(req *http.Request) (bm paymod.BodyMap, err error) {
	...
	bm = make(paymod.BodyMap, len(form)+1)
	for k, v := range form {
		if len(v) == 1 {
			bm.Set(k, v[0])
		}
...
//    返回参数err：错误信息
//    文档：https://opendocs.alipay.com/open/203/105286
func ParseNotifyByURLValues(value url.Values) (bm paymod.BodyMap, err error) {
	bm = make(paymod.BodyMap, len(value)+1)
	for k, v := range value {
		if len(v) == 1 {
			bm.Set(k, v[0])
		}
...
func VerifySign(aliPayPublicKey string, bean interface{}) (ok bool, err error) {
	if aliPayPublicKey == paymod.NULL {
	...
	if reflect.ValueOf(bean).Kind() == reflect.Map {
		if bm, ok = bean.(paymod.BodyMap); ok {
...
func VerifySignWithCert(aliPayPublicKeyPath string, bean interface{}) (ok bool, err error) {
	if aliPayPublicKeyPath == paymod.NULL {
...
func GetCertSN(certPath string) (sn string, err error) {
	...
	if sn == paymod.NULL {
		return paymod.NULL, errors.New("failed to get sn,please check your cert")
	}
	return sn, nil
}
...
func DecryptOpenDataToStruct(encryptedData, secretKey string, beanPtr interface{}) (err error) {
	if encryptedData == paymod.NULL || secretKey == paymod.NULL {
	...
	if len(originData) > 0 {
		originData = paymod.PKCS5UnPadding(originData)
	}
...
//    文档：https://opendocs.alipay.com/mini/introduce/aes
//    文档：https://opendocs.alipay.com/open/common/104567
func DecryptOpenDataToBodyMap(encryptedData, secretKey string) (bm paymod.BodyMap, err error) {
	if encryptedData == paymod.NULL || secretKey == paymod.NULL {
...
func systemOauthToken(appId string, t PKCSType, privateKey string, bm paymod.BodyMap, method string, isProd bool, signType string) (bs []byte, err error) {
	bm.Set("app_id", appId)
	...
	bm.Set("timestamp", time.Now().Format(paymod.TimeLayout))
	bm.Set("version", "1.0")
	var (
		sign    string
		baseUrl = baseUrlUtf8
	...
	_, bs, errs := paymod.NewHttpClient().Type(paymod.TypeForm).Post(baseUrl).SendString(FormatURLParam(bm)).EndBytes()
	if len(errs) > 0 {
		return nil, errs[0]
	}
	return bs, nil
...
func MonitorHeartbeatSyn(appId string, t PKCSType, privateKey, signType, bizContent string) (rsp *MonitorHeartbeatSynResponse, err error) {
	...
	_, bs, errs := paymod.NewHttpClient().Type(paymod.TypeForm).Post(baseUrlUtf8).SendString(FormatURLParam(bm)).EndBytes()
	if len(errs) > 0 {
		return nil, errs[0]
	}
